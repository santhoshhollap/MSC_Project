# Contrastive Learning Based MRI Segmenatation Under Arbitrary Distortion

The objective of this work is to optimise the MRI segmenatation task for arbitrary distortion settings. The proposed network takes the distored MRI image and output the clean segmentation. This is a novel approach to solve the problem which brings a single step solution. Furthermore, this work integrates pixel-wise contrastive learning approach for effcient feature extraction for further improve the accuracy. 

For the network details, experiments and results, please go through the report.pdf

# Content of the repository

This repository has code for pix2pix, cyclegan, cycleMedgan, Custom-Unet and proposed network. Also, this repository has code for a data generation module and validation which has to be executed in the personal systems - python environment. The training and the inference code in the repository is configured for executing in google colab and data has to be uploaded to gdrive. The dependencies and libraries are specified in requirement.txt and environment.yaml.

Contributions and the references are mentioned clearly in the appendix section of the report.
...
5. To train the model
	...
	* Specify the training mode as per which model needs to be trained
       1. Train_mode choose from [pix2pix,cyclegan,cyclemedgan,proposed]
       2. Model_mode choose from [pix2pix,cycle_gan,cycle_mgan,proposed]
	* Model name and path is the user's choice. Specify the hyper-parameters
       1. Batch size = 2 or 4
       2. Learning rate = 0.0002
       3. Number of epochs 1500+ for better results
       4. Patch size 256
